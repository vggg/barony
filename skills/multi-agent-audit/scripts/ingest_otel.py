...
# covers, and barony ADR-021 for the measured contamination it prevents.
...
def partition_guard_records(records):
    """Split (activity_records, baron_records). v1.1; barony ADR-021.
...
GUARD_ONLY_ABSENT = (
    ...
    "(barony ADR-021). The baron rows ARE counted, in `guard_decisions` / "
...
GUARD_PRESENT_SUFFIX = (
    ...
    "(barony ADR-021), so they cannot supply this metric")
...
def compute_guard_metrics(baron_records, src_names):
    """Counts over the withheld barony partition (v1.1; ADR-021).
    ...
    DELIBERATELY ABSENT: a count of `baron.enforcement`. Earlier versions
    withheld it because the attribute was under correction; that correction
    landed (barony ADR-018) and the vocabulary is now `enforced` |
    `unevaluated` | `unknown`, read off an explicit `Decision.adjudicated`
    flag. It is still absent, for a DIFFERENT reason: an honest aggregate must
    filter on `enforcement == "enforced"` BEFORE grouping, because an
    `unevaluated` row still carries a non-empty `baron.capability.verb` and a
    count keyed on the verb alone over-counts (the ADR-018 §5 consumer
    caveat). That filter is un-built and no consumer has asked for it — so
    this is a gap, not a block. ADR-021 §5.
    """
...
def compute_metrics(records, file_reports, session_attr=None):
    ...
    # what leaked when this was missing (barony ADR-021).
