...
Earlier rounds declined to pin `baron.enforcement` because it was under active
correction. **That correction landed (ADR-018)** and this fixture was
regenerated against the corrected producer: the vocabulary is now `enforced` |
`unevaluated` | `unknown`, and no `not-applicable` row survives. The triple is
still not pinned *here*, because the assertion belongs where it can name the
rule — `test_ingest_otel.py` asserts the structural-refusal row is
`unevaluated`, that the fixture speaks only the post-ADR-018 vocabulary, and
that the row still carries a non-empty verb (the ADR-018 §5 consumer caveat).
Outcomes and kinds ARE asserted here: neither is disputed. See ADR-021 §5.

NOTE ON REGENERATION. The published `session_duration_*` figures in ADR-021 §2
are the wall-clock of the hook subprocesses on the machine that generated this
file, so they MOVE on every regeneration. That is not noise to be suppressed —
it is the point of §2, which measures what leaks when guard's own timings are
mistaken for a session. `test_adr021_published_figures_reproduce` fails with
the list of documents to update whenever they move.
...
kinds that carry the same contamination hazard as the decision rows:
 ...
 11. ok        PostToolUse — an EVIDENCE row (`tool.post`). It carries
               ...
               This row is why ADR-021 partitions the whole `baron.`
