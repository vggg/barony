...
**CORRECTED 2026-08-09 (ADR-018).** "Reads baron's own stream with zero new code" was true
...
numbers in ADR-018 §2. The fix is in the CONSUMER — `ingest_otel.partition_guard_records`
...
agent activity (ADR-018). Third-party sinks need no baron change. The default is silent, so
...
- **Cross-version hazard.** An audit-skill ingester older than v1.1 pointed at a
  `.baron/events/*.jsonl` file produces every fabricated number in ADR-018 §2. Baron cannot
