...
| [035](ADR-035-admin-override-role.md) | The admin / override role — authority rooted outside the enforced party, spent as evidence | **Proposed** (2026-08-15) | Design only, no code. Names the admin that already exists de facto (CODEOWNERS + branch protection + ADR-034 L0) and gives `BARON_GUARD_OVERRIDE` a root: an admin principal in a CODEOWNERS-gated registry, reusing ADR-027's trust root, plus signed/scoped/expiring break-glass grants. Depends on **ADR-034** (PR #57). §8 carries seven owner decisions, three blocking. |
...
- Anyone opening a new ADR should start at **034** — and **check the branches, not just this
