...
*Round-4 correction — SUPERSEDED by ADR-018: the claim now rests on four
...
kits were called **unmeasured**. That scoping is now **obsolete**. ADR-018
...
enforce these verbs. See ADR-018 §4.2 and §7.
...
(ADR-018 adds three sibling measurements on the same persona fixture, one per
...
> claim. Recorded in **ADR-018**; §4.1 above is updated, not merely annotated.
>
...
> is outside what `baron rules list` speaks for (ADR-018 §4.2, §7).
>
