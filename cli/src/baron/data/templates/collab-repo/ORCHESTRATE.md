...
> **Shortcut — `baron init` (baron ≥ 0.5.0, ADR-006):** when baron is installed,
> `baron init <name> --dir <collab-dir> [--code-repo <path-or-url>] --personas
> archetype:slug,...` emits steps 1, 2, and 2a deterministically (plus a Tier-1/
> Tier-2 runtime kit per persona) and validates its own output. Resume THIS recipe
> at step 3 (code repo + `.gitignore`) and step 4 (full hydration: real scope
> prose, `AGENT.md`, Tier-3) — init deliberately fills a generic edit-me scope.
