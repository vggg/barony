...
- **`LABEL_CAVEAT` is scoped to what was measured.** It stated "no adapter baron ships does"
  ...
  release by ADR-018 — the other three adapters were measured and the "unmeasured" scoping
  is retired. See the ADR-018 entry below.)*
...
### Changed — the read-verb posture label now rests on FOUR measured adapters ([ADR-018](docs/adr/ADR-018-read-verb-posture-measured-on-four-adapters.md))
...
- **The honest bound is published with the label**: the measured claim is *baron emits no
  ...
  Recorded as a known divergence (ADR-018 §7), not resolved by editing one table to match
  the other.
