...
- **guard-only export** — `session_count` 1, `session_duration_total_s` 1.096 (the wall-clock
...
- **paired with `flat_spans.jsonl`** — nine activity metrics moved: `session_count` 1 → 2,
  `session_duration_p50_s` **600.0 → 300.548**, `tool_calls_total` 1 → 12, `tool_error_rate`
