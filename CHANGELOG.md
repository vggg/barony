...
### Fixed — baron's own evidence was being counted as agent activity (ADR-018)
...
- **guard-only export** — `session_count` 1, `session_duration_total_s` 0.91 (the wall-clock
...
- **paired with `flat_spans.jsonl`** — nine activity metrics moved: `session_count` 1 → 2,
  `session_duration_p50_s` **600.0 → 300.455**, `tool_calls_total` 1 → 12, `tool_error_rate`
  ...
  arrival of evidence. Eleven metrics move on `otlp_two_sessions.json`, seven on
  `missing_attrs.jsonl`.
...
- **NOT shipped: an aggregate over `baron.enforcement`.** `harden/otel` had one; it is not
  ...
  count over it would look authoritative. It lands when the label does — ADR-018 §5.
